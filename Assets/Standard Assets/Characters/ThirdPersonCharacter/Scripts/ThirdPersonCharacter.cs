...
public class ThirdPersonCharacter : MonoBehaviour
{
	...
	[SerializeField] protected float m_JumpPower = 6f;
	...
	protected virtual void ScaleCapsuleForCrouching(bool crouch)
	{
		...
		else
		{
			...
			if (Physics.SphereCast(crouchRay, m_Capsule.radius * k_Half, crouchRayLength, PhysCheck, QueryTriggerInteraction.Ignore))
			{
				m_Crouching = true;
				return;
			}
	...
	protected virtual void CheckGroundStatus()
	{
		...
		if (Physics.Raycast(transform.position + (Vector3.up * 0.1f), Vector3.down, out hitInfo, m_GroundCheckDistance, PhysCheck))
		{
